...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet:
  ...
  override def mostRetweeted: Tweet = {
    def _max(x: Tweet, y: Tweet): Tweet = if (x.retweets >= y.retweets) then x else y

    if (left.isEmpty && right.isEmpty) then
      elem
    else if (left.isEmpty) then
      _max(elem, right.mostRetweeted)
    else if (right.isEmpty) then
      _max(elem, left.mostRetweeted)
    else
      _max(left.mostRetweeted, _max(elem, right.mostRetweeted))
  }
