...
trait Huffman extends HuffmanInterface:
  ...
  def times(chars: List[Char]): List[(Char, Int)] = {
    def iter(map: Map[Char, Int], c: Char) = {
   ...
   */
  def singleton(trees: List[CodeTree]): Boolean = if (trees.length == 1) then true else false
  ...
  def codeBits(table: CodeTable)(char: Char): List[Bit] = {
    table.filter(ct => ct._1 == char).head._2
  }
  ...
  def mergeCodeTables(a: CodeTable, b: CodeTable): CodeTable = {
    a.map(ct => (ct._1, 0 :: ct._2)) ::: b.map(ct => (ct._1, 1 :: ct._2))
  }
